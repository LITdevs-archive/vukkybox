	...
	if(req.board != "rarity") {
		...
		User.find({}, function(err, allUsers){
			...
			for (let i = 0; i < allUsers.length; i++) {
				...
				console.log({username: allUsers[i].username.includes("@") ? "Username Hidden for Privacy" : allUsers[i].username, userId: allUsers[i]._id, data: vukkyTierCount(allUsers[i].gallery)[req.rarity]})
				console.log(req.rarity)
				if (i + 1 == allUsers.length) {
					//last loop
