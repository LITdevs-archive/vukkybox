...
app.post('/leaderboard', grl, function(req, res) {
	...
	if(validBoards.includes(req.body.board) && req.body.limit != undefined && parseInt(req.body.limit) > 0 && parseInt(req.body.limit) <= 200) {
		db.leaderboard({limit: parseInt(req.body.limit), board: req.body.board}, user, response => {
			res.send(response);
		})
	} else {
...
app.get('/leaderboard', grl, function(req, res) {
	...
	console.log(`user ${user} req.user_id ${req.user_id} req.user ${req.user} req.isAuthenticated ${req.isAuthenticated()} ${req.user_id ? req.user : req.user[0]}`)
